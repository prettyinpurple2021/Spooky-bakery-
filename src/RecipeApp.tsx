...
import { collection, doc, query, where, getDocs, addDoc, updateDoc, deleteDoc, serverTimestamp } from "firebase/firestore";
...
const DEFAULT_CATEGORIES = ["Cake", "Breakfast", "Dessert", "Other"];

function parseCookTimeToSeconds(cookStr: string): number {
  if (!cookStr) return 0;
  
  // Try to find hours first, e.g., "1 hour", "1.5 hours", "2 h", "2.5hrs"
  const hourRegex = /(\d+(?:\.\d+)?)\s*(?:hour|hours|hr|hrs|h)\b/i;
  const hourMatch = cookStr.match(hourRegex);
  
  // Try to find minutes, e.g., "25 mins", "25 minutes", "25m", "25 min"
  const minRegex = /(\d+(?:\.\d+)?)\s*(?:min|mins|minute|minutes|m)\b/i;
  const minMatch = cookStr.match(minRegex);
  ...
  if (minMatch) {
    ...
    // If no explicit unit matched but there's a number at the start, e.g. "25"
    const simpleNumRegex = /^\s*(\d+(?:\.\d+)?)\s*(?:@|$|\D)/;
    const simpleMatch = cookStr.match(simpleNumRegex);
    if (simpleMatch) {
      totalSeconds += parseFloat(simpleMatch[1]) * 60; // assume minutes
    }
  }
...
export default function RecipeApp() {
  ...
  const [formCategory, setFormCategory] = useState<string>("Cake");
  ...
  const [formImageSize, setFormImageSize] = useState("1K");
  ...
  // Fetch all recipes and categories on load
  useEffect(() => {
    fetchRecipes();
    fetchCategories();
    triggerBubbles();
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      const data = snaps.docs.map(doc => ({ id: doc.id, name: doc.data().name as string }));
  ...
  const addCategory = async (name: string) => {
    ...
    // Check duplicate
    const exists = [...DEFAULT_CATEGORIES, ...customCategories.map(c => c.name)].some(
    ...
    try {
      ...
      showToast(`Category "${cleanName}" crafted successfully.`);
      setNewCategoryName("");
  ...
  const renameCategory = async (catId: string, oldName: string, newName: string) => {
    if (!user) return;
    ...
    // Check duplicate
    const exists = [...DEFAULT_CATEGORIES, ...customCategories.map(c => c.name)].some(
      c => c.toLowerCase() === cleanNewName.toLowerCase()
    );
    if (exists) {
      showToast(`Category "${cleanNewName}" already exists.`);
      return;
    }

    try {
      // 1. Update the category doc
      await updateDoc(doc(db, "categories", catId), { name: cleanNewName });

      // 2. Update existing recipes of this category
      const recipesToUpdate = recipes.filter(r => r.category === oldName);
      for (const recipe of recipesToUpdate) {
        if (recipe.id) {
          const docRef = doc(db, "recipes", recipe.id);
          await updateDoc(docRef, { category: cleanNewName });
        }
      }

      showToast(`Category renamed to "${cleanNewName}". Recipes updated.`);
      setEditingCategoryId(null);
      await fetchCategories();
      await fetchRecipes();
    } catch (err: any) {
      showToast(err.message || "Failed to rename category.");
    }
  };
  ...
  const deleteCategory = async (catId: string, catName: string) => {
    ...
    try {
      ...
      // 2. Reassign recipes to "Other"
      const recipesToUpdate = recipes.filter(r => r.category === catName);
      for (const recipe of recipesToUpdate) {
        if (recipe.id) {
          const docRef = doc(db, "recipes", recipe.id);
          await updateDoc(docRef, { category: "Other" });
        }
      }

      showToast(`Category "${catName}" banished. Recipes reassigned to "Other".`);
      await fetchCategories();
      await fetchRecipes();
    } catch (err: any) {
  ...
  const fetchRecipes = async () => {
    ...
    try {
      ...
      const snaps = await getDocs(q);
      const data: Recipe[] = snaps.docs.map(doc => ({ id: doc.id, ...doc.data() } as Recipe));
      setRecipes(data);
      setError(null);
    } catch (err: any) {
      console.error(err);
      setError(err.message || "An unexpected error occurred reading the spellbook.");
  ...
  const toggleFavorite = async (index: number) => {
    ...
    const docPath = `recipes/${recipe.id}`;
    try {
      const docRef = doc(db, "recipes", recipe.id);
      ...
      setRecipes(prev => {
        const next = [...prev];
        next[index] = { ...recipe, favorite: !recipe.favorite };
        return next;
      });
      showToast(!recipe.favorite ? "Recipe spell bookmarked!" : "Recipe spell un-bookmarked.");
    } catch (err: any) {
  ...
  const openRecipeEditForm = (index: number) => {
    ...
    setFormImageUrl(rc.imageUrl || "");
    setIsFormOpen(true);
  };

  const generateCoverImage = async () => {
    if (!formTitle) {
      showToast("Please enter a title first so we know what to summon!");
      return;
    }
    setIsGeneratingImage(true);
    try {
      const prompt = `A creepy kawaii pastel goth dessert, ${formTitle}, cute spooky aesthetic, glowing pink and mint green, dark aesthetic, 3d render.`;
      const res = await fetch("/api/images/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt, size: formImageSize, aspectRatio: "4:3", model: "gemini-3.1-flash-image" })
      });
      if (!res.ok) throw new Error("Our magic mirror failed to return an image.");
      const data = await res.json();
      setFormImageUrl(data.imageUrl);
      showToast("A new spectral image has materialised!");
    } catch (err: any) {
      showToast(err.message || "Could not conjure visual illusion.");
    } finally {
  ...
  const saveRecipeSpell = async (e: FormEvent) => {
    ...
    let docPath = "recipes";
    let isUpdate = formIndex !== null;

    try {
      if (!user) throw new Error("Unauthenticated apprentice!");
      ...
      if (formIndex !== null) {
        const existingId = recipes[formIndex]?.id;
        if (existingId) {
          docPath = `recipes/${existingId}`;
          const docRef = doc(db, "recipes", existingId);
          await updateDoc(docRef, payload);
        }
        showToast("Recipe spell modified successfully.");
      } else {
        payload.favorite = false;
        payload.createdAt = serverTimestamp();
        await addDoc(collection(db, "recipes"), payload);
        showToast("New recipe written onto the recipe deck!");
      }
      ...
      handleFirestoreError(err, isUpdate ? OperationType.UPDATE : OperationType.CREATE, docPath);
    }
  };
  ...
  const confirmBanishRecipe = async () => {
    ...
    const recipe = recipes[banishIndex];
    if (!recipe || !recipe.id) {
       setBanishIndex(null);
       return;
    }
    ...
    try {
      ...
      setRecipes(prev => prev.filter((_, idx) => idx !== banishIndex));
    ...
    <div className="min-h-screen text-[#e2d9f3] relative select-none selection:bg-[#ffb7c5] selection:text-[#120b1c] font-sans">
      ...
      <header className="no-print bg-[#171324] border-b-4 border-[#ffb7c5]/30 pb-4 px-4 md:px-8 shadow-2xl sticky top-0 z-30">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6 py-6">
          ...
          <div className="flex flex-wrap gap-4 relative z-30 justify-center items-center">
            <button 
              id="add-spell-btn"
            ...
            <Link 
              to="/settings" 
              className="px-5 py-3 bg-[#1e172e] border-2 border-[#c397e8] hover:bg-[#2b1f42] text-[#c397e8] font-bold rounded-2xl transition-all text-sm flex items-center gap-2 shadow-[0_0_15px_rgba(195,151,232,0.2)] cursor-pointer"
            >
              ...
              {[
                ...
                return (
                  ...
                  >
                    ...
                    {cat === "Cake" ? "Cakes & Frosting" : cat === "Breakfast" ? "Witching Hours" : cat === "Dessert" ? "Sweet Sorcery" : cat === "Other" ? "Baking Staples" : cat}
                  </button>
                );
              })}

              {/* Category manager launcher */}
              <button
                onClick={() => setIsCategoryManagerOpen(true)}
                className="px-3 py-1.5 rounded-xl text-xs font-bold bg-[#130f1f]/80 hover:bg-[#1e172e] text-[#98ffd9] border border-[#98ffd9]/30 flex items-center gap-1 cursor-pointer transition-all hover:scale-105"
                title="Manage categories grimoire"
              >
                <SettingsIcon size={12} className="animate-spin" style={{ animationDuration: "8s" }} />
                <span>Categories Grimoire</span>
              </button>
            </div>
          </div>
        ...
        <section id="cupboard-and-audio-console" className="no-print grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
          ...
          <div className="lg:col-span-8 glass-card rounded-3xl p-6 border-2 border-[#ffb7c5]/30 shadow-[0_10px_30px_rgba(255,183,197,0.1)] flex flex-col justify-between">
            <div>
              ...
              {/* Stocked Reagents Shelf with Entrance Animations */}
              <div className="mt-5 mb-3">
                <h4 className="text-[11px] font-extrabold uppercase tracking-widest text-[#98ffd9]/85 mb-2.5 flex items-center gap-1.5 font-spooky">
                  <span>Stocked Cabinet Reagents</span>
                  <span className="w-1.5 h-1.5 bg-[#98ffd9] rounded-full animate-ping" />
                </h4>
                <div className="p-3 bg-[#130f1f]/50 border-2 border-[#ffb7c5]/15 rounded-2xl min-h-[58px] flex flex-wrap gap-2 items-center relative overflow-hidden backdrop-blur-sm shadow-inner">
                  <AnimatePresence mode="popLayout">
                    {pantryIngredients.length === 0 ? (
                      <motion.p
                        ...
                        className="text-[11px] text-stone-400 italic text-center w-full"
                      >
                        Your alchemical cupboard is completely bare...
                      </motion.p>
                    ) : (
                      pantryIngredients.map((item) => (
                        <motion.div
                          ...
                          layout
                          onClick={() => togglePantryIngredient(item)}
                          className="group relative px-2.5 py-1.5 rounded-xl bg-[#1e172e] border border-[#ffb7c5]/30 hover:border-[#ffb7c5] shadow-[0_3px_8px_rgba(255,183,197,0.06)] hover:shadow-[0_0_12px_rgba(255,183,197,0.25)] transition-all flex items-center gap-1.5 text-xs font-bold text-[#ffb7c5] cursor-pointer"
                          title="Banish reagent from cupboard"
                        >
                          <span className="w-1.5 h-1.5 bg-[#ffb7c5] rounded-full group-hover:bg-[#98ffd9] group-hover:scale-125 transition-all" />
                          <span>{item}</span>
                          <span className="text-[10px] text-stone-500 group-hover:text-[#ff9898] transition-colors ml-1 font-sans">×</span>
                        </motion.div>
                      ))
                    )}
                  </AnimatePresence>
            ...
            {filteredRecipes.length === 0 ? (
              ...
              filteredRecipes.map((recipe, index) => {
                ...
                return (
                  ...
                  >
                    <div className="relative h-40 w-full overflow-hidden bg-[#1e172e] border-b border-[#ffb7c5]/20">
                      <div className="absolute inset-0 bg-gradient-to-tr from-[#120b1c]/80 via-transparent to-transparent z-20 pointer-events-none"></div>
                      ...
                      <div className="w-full h-full opacity-80 group-hover:scale-110 transition-transform duration-700">
                        <PastelGothMagic type={["cauldron","book","potion","bakery","ghost","bat"][index % 6] as any} className="w-full h-full" />
                      </div>
                    </div>
                    <div className="p-6 pb-4 border-b border-dashed border-[#ffb7c5]/20 bg-[#130f1f]/50 relative">
                      ...
                      <div className="flex flex-wrap gap-2 items-center mb-2">
                        <span className="inline-block px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-[#c397e8]/20 text-[#c397e8] border border-[#c397e8]/30">
                          {recipe.category === "Cake" ? "Cakes & Frosting" : recipe.category === "Breakfast" ? "Witching Hours" : recipe.category === "Dessert" ? "Sweet Sorcery" : recipe.category === "Other" ? "Baking Staples" : recipe.category}
                        </span>
                      ...
                      <div className="flex flex-col sm:flex-row gap-4 pt-4 border-t border-stone-850/60 items-center justify-between">
                        <div className="flex gap-2 w-full sm:flex-1">
                          <button 
                            ...
                            className="flex-1 px-4 py-3 btn-3d-pink font-bold rounded-2xl text-xs flex items-center justify-center gap-1.5 cursor-pointer"
                          >
                          ...
                          <button 
                            onClick={() => {
                              const secs = parseCookTimeToSeconds(recipe.cook);
                              if (secs > 0) {
                                setTimerPreset(secs);
                                setIsTimerRunning(true);
                                showToast(`🔮 Baking cauldron timer ignited for ${recipe.title}: ${Math.floor(secs / 60)}m!`);
                              } else {
                                const defaultSecs = 1200; // 20 mins
                                setTimerPreset(defaultSecs);
                                setIsTimerRunning(true);
                                showToast(`🔮 No custom bake time detected. Default 20m timer ignited!`);
                              }
                            }}
                            className="flex-1 px-4 py-3 btn-3d-mint font-bold rounded-2xl text-xs flex items-center justify-center gap-1.5 cursor-pointer"
                            title="Quickly start baking cauldron timer"
                          >
                            <Clock size={14} className="animate-pulse" /> Start Timer
                          </button>
                        </div>
      ...
      {activeBakingRecipe && (
        <div className="no-print fixed inset-0 bg-[#0c0a12]/95 backdrop-blur-xl z-50 flex items-center justify-center p-4 transition-all duration-300">
          <div className="bg-[#171324] rounded-3xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col shadow-[0_0_50px_rgba(255,183,197,0.35)] border-4 border-[#ffb7c5] transform scale-100 transition-transform text-[#e2d9f3]">
            ...
            {/* Header */}
            <div className="relative p-6 border-b border-stone-850 flex justify-between items-center bg-[#130f1f] overflow-hidden">
              <div className="absolute inset-0 bg-[#c397e8]/30 mix-blend-color z-10 pointer-events-none"></div>
              <div className="absolute inset-0 bg-[#ffb7c5]/20 mix-blend-multiply z-10 pointer-events-none"></div>
              <div className="absolute inset-0 bg-gradient-to-r from-[#171324] via-[#171324]/80 to-transparent z-10 pointer-events-none"></div>
              <div className="absolute inset-0 w-full h-full opacity-40 z-0 pointer-events-none">
                <PastelGothMagic type="cauldron" className="w-full h-full" />
              </div>
              ...
              <button 
                ...
                className="relative z-20 text-stone-400 hover:text-white transition-all bg-[#171324]/80 backdrop-blur border border-[#37284d] hover:bg-[#1e172e] p-2.5 rounded-2xl cursor-pointer"
              >
                <X size={20} />
              </button>
            </div>
        ...
        <div className="no-print fixed inset-0 bg-[#0c0a12]/95 backdrop-blur-xl z-50 flex items-center justify-center p-4">
          <div className="bg-[#171324] rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-[0_0_50px_rgba(195,151,232,0.35)] border-4 border-[#c397e8] transform scale-100 transition-transform text-[#e2d9f3]">
            ...
            <form onSubmit={saveRecipeSpell} className="overflow-y-auto p-6 space-y-4 flex-1">
              ...
              <div className="flex flex-col gap-4 bg-[#130f1f] p-4 rounded-xl border-2 border-[#1e172e]">
                <div className="flex items-center gap-4">
                  {formImageUrl ? (
                     <img src={formImageUrl} alt="Visual Illusion" className="h-24 w-24 object-cover rounded-xl" />
                  ) : (
                    <div className="h-24 w-24 flex items-center justify-center bg-[#0c0a12] border border-[#37284d] rounded-xl text-stone-600 text-xs text-center p-2">
                      No visual apparition generated yet.
                    </div>
                  )}
                  <div className="flex-1 space-y-3">
                    <label className="block text-xs font-bold uppercase tracking-wider text-[#c397e8]">Spell Visual Apparition</label>
                    <div className="flex items-center gap-3">
                      <select 
                        value={formImageSize}
                        onChange={(e) => setFormImageSize(e.target.value)}
                        className="px-3 py-2 bg-[#171324] border-2 border-stone-750 text-sm text-[#c397e8] font-bold rounded-xl outline-none"
                      >
                      ...
                      <button
                        ...
                        disabled={isGeneratingImage || !formTitle}
                        className="px-4 flex items-center justify-center gap-2 py-2 btn-3d-mint font-bold rounded-xl text-xs flex-1 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {isGeneratingImage ? "Summoning Mirror..." : <><Sparkles size={14} /> Conjure Visage (AI)</>}
                      </button>
                    </div>
                  </div>
                </div>
      ...
      {/* CATEGORY GRIMOIRE MANAGER DIALOG */}
      {isCategoryManagerOpen && (
        <div className="no-print fixed inset-0 bg-[#0c0a12]/95 backdrop-blur-xl z-50 flex items-center justify-center p-4">
          <div className="bg-[#171324] rounded-3xl w-full max-w-md max-h-[95vh] overflow-hidden flex flex-col shadow-[0_0_50px_rgba(195,151,232,0.35)] border-4 border-[#98ffd9] transform scale-100 transition-transform text-[#e2d9f3]">
            
            <div className="p-6 border-b border-stone-850 flex justify-between items-center bg-[#130f1f]">
              <div className="flex items-center gap-2">
                <SettingsIcon className="text-[#98ffd9] animate-spin" style={{ animationDuration: "12s" }} size={20} />
                <h3 className="spooky-title text-xl font-bold text-[#98ffd9] glow-mint">
                  Category Grimoire
                </h3>
              </div>
              <button 
                ...
                className="text-stone-400 hover:text-white transition-all bg-stone-850 p-2 rounded-xl cursor-pointer"
              >
                <X size={16} />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1">
              ...
              <div className="space-y-2">
                <label className="block text-xs font-bold uppercase tracking-wider text-[#c397e8]">
                  Forge New Spell Category
                </label>
                <div className="flex gap-2">
                  <input 
                    ...
                    className="flex-1 px-4 py-2 bg-[#130f1f] border-2 border-stone-750 rounded-xl focus:outline-none focus:border-[#98ffd9] text-sm text-white"
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-[#98ffd9] text-[#130f1f] font-bold rounded-xl text-xs uppercase tracking-wide hover:bg-[#a9ffe0] transition-colors"
                  >
                    Forge
                  </button>
                </div>
              ...
              {/* Categories lists */}
              <div className="space-y-4">
                <h4 className="text-xs font-bold uppercase tracking-wider text-[#ffb7c5]">
                  Active Classifications
                </h4>
                
                <div className="space-y-2.5 max-h-[45vh] overflow-y-auto pr-1">
                  {/* Default Categories (Static/Locked) */}
                  {DEFAULT_CATEGORIES.map((cat) => (
                    <div 
                      key={cat} 
                      className="flex items-center justify-between p-3 bg-[#130f1f]/50 border border-stone-800 rounded-xl"
                    >
                      <span className="text-sm font-medium text-stone-400 flex items-center gap-2">
                        <span>{cat === "Cake" ? "Cakes & Frosting" : cat === "Breakfast" ? "Witching Hours" : cat === "Dessert" ? "Sweet Sorcery" : cat === "Other" ? "Baking Staples" : cat}</span>
                        <span className="text-[10px] uppercase font-bold text-[#c397e8]/60 bg-[#c397e8]/5 px-2 py-0.5 rounded border border-[#c397e8]/10">Default</span>
                      </span>
                      <div className="text-stone-600" title="Locked classification">
                        <Lock size={12} />
                      </div>
                    </div>
                  ))}
                  ...
                  {customCategories.map((cat) => {
                    ...
                    return (
                      <div 
                        key={cat.id} 
                        className="flex items-center justify-between p-3 bg-[#130f1f]/80 border-2 border-[#c397e8]/20 rounded-xl hover:border-[#ffb7c5]/40 transition-all"
                      >
                        {isEditing ? (
                          <div className="flex-1 flex gap-2">
                            <input 
                              ...
                              className="flex-1 px-2.5 py-1 bg-[#171324] border border-[#ffb7c5] rounded-lg text-sm text-white focus:outline-none"
                              autoFocus
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                                  renameCategory(cat.id, cat.name, editingCategoryName);
                            ...
                            />
                            <button
                              onClick={() => renameCategory(cat.id, cat.name, editingCategoryName)}
                              className="px-2 py-1 bg-[#98ffd9] text-[#130f1f] rounded-lg text-xs font-bold"
                            >
                              Save
                            </button>
                            <button
                              onClick={() => setEditingCategoryId(null)}
                              className="px-2 py-1 bg-stone-800 text-stone-300 rounded-lg text-xs font-bold"
                            >
                              Cancel
                            </button>
                          </div>
                          ...
                          <>
                            <span className="text-sm font-bold text-[#ffb7c5]">
                              {cat.name}
                            </span>
                            <div className="flex gap-2">
                              <button
                                ...
                                className="p-1.5 hover:bg-stone-850 rounded-lg text-stone-400 hover:text-[#98ffd9] transition-colors"
                              ...
                              <button
                                ...
                                className="p-1.5 hover:bg-stone-850 rounded-lg text-stone-400 hover:text-red-400 transition-colors"
            ...
            <div className="bg-[#130f1f] p-5 border-t border-stone-850 flex justify-end">
              <button 
                onClick={() => {
                  setIsCategoryManagerOpen(false);
                  setEditingCategoryId(null);
                }} 
                className="px-5 py-2.5 bg-stone-800 hover:bg-stone-700 text-stone-200 font-bold rounded-2xl text-xs uppercase tracking-wider"
              >
                Close Grimoire
              </button>
            </div>
      ...
      <Chatbot onRecipeSaved={async (r) => { 
        if (r && user) {
          try {
            await addDoc(collection(db, "recipes"), { ...r, ownerId: user.uid, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
          } catch (err) {
            console.error(err);
        ...
        fetchRecipes(); 
      }} pantry={pantryIngredients} recipes={recipes} />
    </div>
  );
}
